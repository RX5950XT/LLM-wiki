...
- workspace 生命週期工具**不再被 strip**：pipeline 可 `renameWorkspace` / `createWorkspace` / `deleteWorkspace` / `reorderWorkspaces` / `movePageToWorkspace`。Prompt 硬性要求：只刪空工作區（頁面先搬走）、**永不刪除觸發當下的 workspace**（否則使用者會停在死頁）。
...
- **維護 job 只能刪「本輪開始時就已經是空的」工作區（`deletableWorkspaceIds`，勿移除）**：實測 gemini-3.5-flash 把「合併工作區」理解成「把所有頁面掃進我被觸發的那個工作區，再刪掉空殼」——一輪就把「個人理財與退休規劃」14 頁全倒進「地緣政治與全球貿易」然後刪掉整個工作區（頁面沒丟，但使用者少了一個書架，畫面只顯示「已變更 N 項」）。`organize-pipeline` 依 inventory 算出「知識頁數 = 0」的工作區集合傳給 `buildWikiTools`，`deleteWorkspace` 只接受集合內的 id。合法的合併不受影響：這一輪把頁面搬走，下一輪（`more_work` 自動接力）看到空殼就刪。prompt 也補上「每次搬移必須讓目標工作區更內聚」「當前工作區不是垃圾桶」「不同主題的工作區不可合併」。測試：`apps/web/lib/ai/tools.test.ts`（`bun test`）。
...
`gateDestructive` 只在 `confirmDestructive: true` 時生效（對話流程）。維護 job 走 `confirmDestructive: false`（按鈕 confirm = 授權），刪除直接執行；ingest pipeline 沒有 workspace 工具，不受影響。
