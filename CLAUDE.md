...
│   ├── organize/                   ← POST/GET: 跨工作區自動分類＋去重複 job（agent_jobs）
...
└── lib/ai/
    ...
    ├── organize-pipeline.ts    ← 跨工作區去重＋重新分類 pipeline
...
- `Wand2` 按鈕觸發 POST `/api/organize`（跨全部工作區自動分類＋去重複）
...
背景 job（organize）沒有 UI 可確認：`gateDestructive` 在沒有 `onProposal` 時直接拒絕動作，並要求模型改成在報告中列出「建議刪除」。

## 自動分類＋去重複（organize job）

跨**全部工作區**的長任務，`agent_jobs` 表（migration `0015`，已套用 production）：
- `POST /api/organize` `{ workspace_id }` → 檢查同使用者無 running job → 建 job → `after()` 跑 `runOrganizePipeline` → `202 { jobId }`
- `GET /api/organize?job_id=` → `{ status, progress, report_workspace_id, report_slug }`，8 分鐘 stale sweep
- pipeline 給模型看「所有工作區 + 各自頁面清單」，要求：找重複 → 合併 → `movePageToWorkspace` 歸位錯置頁 → 更新各 index.md → 寫報告頁 `_organize/YYYYMMDD.md`（kind `lint`）到觸發的工作區
- 入口：Web 頂列 `Wand2` 按鈕（confirm → 輪詢 → 完成跳報告頁）；Android drawer 底列 `AutoFixHigh` icon
